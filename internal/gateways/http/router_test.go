...
func TestSensorHistory(t *testing.T) {
	t.Run("GET_sensors_sensor_id/history", func(t *testing.T) {
		t.Run("ok_200", func(t *testing.T) {
			w := httptest.NewRecorder()

			startDate := time.Now().Add(-10 * time.Second).Format(time.RFC1123)
			endDate := time.Now().Add(10 * time.Second).Format(time.RFC1123)
			req, _ := http.NewRequest(http.MethodGet, fmt.Sprintf("/sensors/1/history?start_date=%s&end_date=%s", startDate, endDate), nil)
			req.Header.Add("Accept", "application/json")

			router.ServeHTTP(w, req)

			var events []models.HistoryOfEvents
			assert.NoError(t, json.Unmarshal(w.Body.Bytes(), &events))
			assert.Len(t, events, 1)
			assert.Equal(t, *events[0].Payload, int64(10))
			assert.Equal(t, http.StatusOK, w.Code, "Получили в ответ не тот код")
			assert.True(t, json.Valid(w.Body.Bytes()), "В ответе не json")
		})
